...
TEST(predict_ctrv, buildPredictionFromCTRVState)
{
  ...
  cav_msgs::msg::PredictedState result = buildPredictionFromCTRVState(state, pose, twist);
...
{
  cav_msgs::msg::ExternalObject obj;
  obj.header.stamp = rclcpp::Time(5.0);
  ...
  cav_msgs::msg::PredictedState result = motion_predict::ctrv::predictStep(obj, 0.1, 1000, 0.99);

  // Verify update functions were called
  ASSERT_NEAR(5.0, result.predicted_position.position.x, 0.00001);
  ASSERT_NEAR(0.99, result.predicted_position_confidence, 0.01);
  ASSERT_NEAR(0.001, result.predicted_velocity_confidence, 0.001);
  rclcpp::Time new_time = obj.header.stamp + rclcpp::Duration(0.1);
...
{
  cav_msgs::msg::PredictedState obj;
  obj.header.stamp = rclcpp::Time(5.0);
  obj.header.frame_id = "my_frame";
  ...
  cav_msgs::msg::PredictedState result = motion_predict::ctrv::predictStep(obj, 0.1, 0.99);
...
TEST(predict_ctrv, predictPeriod)
{
  ...
  std::vector<cav_msgs::msg::PredictedState> results = motion_predict::ctrv::predictPeriod(
    obj, 0.1,
    0.21, 1000,
    0.99);

  // Verify update functions were called
  ASSERT_NEAR(5.1, results[0].predicted_position.position.x, 0.00001);
  ...
  // Verify update functions were called
  ASSERT_NEAR(5.2, results[1].predicted_position.position.x, 0.00001);
  ...
  new_time = obj.header.stamp + rclcpp::Duration(0.2);
