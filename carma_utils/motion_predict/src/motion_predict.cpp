...
Mapping(const double input, const double process_noise_max)
{
  ...
  double output = (input - input_start) / (input_end - input_start) *
    (output_end - output_start) + output_start;

  return output;
}

cav_msgs::msg::PredictedState
predictState(
  const geometry_msgs::msg::Pose & pose,
  const geometry_msgs::msg::Twist & twist, const double delta_t)
{
  ...
  // Generate identity matrix for state transition matrix
  ...
  cav_msgs::msg::PredictedState pobj;
...
cav_msgs::msg::PredictedState
externalPredict(
  const cav_msgs::msg::ExternalObject & obj, const double delta_t,
  const double ax, const double ay,
  const double process_noise_max)
{
  cav_msgs::msg::PredictedState pobj = predictState(obj.pose.pose, obj.velocity.twist, delta_t);
  ...
  Q << (delta_t4 / 4 * ax), 0, ( delta_t3 / 2 * ax), 0, 0, (delta_t4 / 4 * ay), 0,
  ( delta_t3 / 2 * ay), (delta_t3 / 2 * ax), 0, (delta_t2 * ax), 0, 0, (delta_t3 / 2 * ay), 0,
  (delta_t2 * ay);  // Process Noise Matrix

  // Transpose of State Transition Function
  Eigen::MatrixXd Ft = F.transpose();

  // State Covariance Matrix
  P = F * P * Ft + Q;

  // Position process noise average
  double position_process_noise_avg = (P(0, 0) + P(1, 1)) / 2;

  // Position process noise confidence
  pobj.predicted_position_confidence = Mapping(position_process_noise_avg, process_noise_max);

  // Position velocity process noise average
  double velocity_process_noise_avg = (P(2, 2) + P(3, 3)) / 2;

  // Velocity process noise confidence
...
cav_msgs::msg::PredictedState
predictStep(
  const cav_msgs::msg::PredictedState & obj, const double delta_t,
  const double confidence_drop_rate)
{
  // Predict Motion
  cav_msgs::msg::PredictedState pobj = predictState(
    obj.predicted_position, obj.predicted_velocity,
    delta_t);
...
std::vector<cav_msgs::msg::PredictedState>
predictPeriod(
  const cav_msgs::msg::ExternalObject & obj,
  const double delta_t, const double period,
  const double ax, const double ay,
  const double process_noise_max,
  const double confidence_drop_rate)
{
  std::vector<cav_msgs::msg::PredictedState> predicted_states = {externalPredict(
      obj, delta_t, ax, ay,
      process_noise_max)};

  double t = delta_t;
  while (t < period) {
    predicted_states.emplace_back(
      predictStep(
        predicted_states.back(), delta_t, confidence_drop_rate));
    t += delta_t;
  }
