...
localVelOrientationAndMagnitude(const double v_x, const double v_y)
{
...
CTRV_State
buildCTRVState(const geometry_msgs::msg::Pose & pose, const geometry_msgs::msg::Twist & twist)
{
  ...
  // The yaw is relative to the velocity vector so take the heading and
  // add it to the angle of the velocity vector in the local frame
  state.yaw = rpy[2] + std::get<0>(vel_angle_and_mag);
...
buildPredictionFromCTRVState(
  const CTRV_State & state, const geometry_msgs::msg::Pose & original_pose,
  const geometry_msgs::msg::Twist & original_twist)
{
  ...
  auto vel_angle_and_mag = localVelOrientationAndMagnitude(
    original_twist.linear.x,
...
CTRV_State
CTRVPredict(const CTRV_State & state, const double delta_t)
{
  ...
  const float process_noise_max, const double confidence_drop_rate)
{
  ...
  cav_msgs::msg::PredictedState pobj = buildPredictionFromCTRVState(
  ...
  pobj.predicted_position_confidence = motion_predict::cv::Mapping(
    position_process_noise_avg,
    process_noise_max) *
    confidence_drop_rate;
  pobj.predicted_velocity_confidence = motion_predict::cv::Mapping(
    velocity_process_noise_avg,
    process_noise_max) *
    confidence_drop_rate;
...
predictPeriod(
  ...
  const double confidence_drop_rate)
{
  std::vector<cav_msgs::msg::PredictedState> predicted_states = {predictStep(
      obj, delta_t, process_noise_max,
      confidence_drop_rate)};
